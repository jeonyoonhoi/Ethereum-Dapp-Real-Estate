...
contract('RealEstate',function(accounts) {
    ...
    it("가나슈 두 번째 계정으로 매물 아이디 0번 매입 후 이벤트 생성 및 매입자 정보와 buyers 배열 테스팅", function() {
        return RealEstate.deployed().then(function(instance) {
            ...
            assert.equal(buyerInfo[0].toUpperCase(), accounts[1].toUpperCase(), "매입자의 계정이 가나슈 두번째 계쩡과 일치하지 않습니다. ")
            assert.equal(web3.toAscii(buyerInfo[1]).replace(/\0/g,''), "yoonhoi","매입자의 이름이 sejong이 아닙니다. ");
